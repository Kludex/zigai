...
`zig build` installs one unified production client and three provider-specific
smoke clients:
...
The unified `zigai` client adds strict JSON configuration, stdin (`-`), atomic
conversation history, multiple MCP stdio servers, resumable approval files,
text/JSON/AG-UI event output, stable exit codes, and bash/zsh/fish completion:
...
Browser-controlled or model-controlled data never selects credential values.
Configuration names only the environment variable that contains a credential.
