...
pub fn main(init: std.process.Init) !void {
    ...
    const url_policy = common.urlPolicyForConfiguredEndpoint(base_url, zigai.openai.api_base);
    var http = zigai.transport.HttpTransport.initWithOptions(init.gpa, init.io, .{
        .url_policy = url_policy,
    });
    defer http.deinit();
    var provider = zigai.openai.Provider.initWithOptions(input.api_key, http.transport(), .{
        .base_url = base_url,
