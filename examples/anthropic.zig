...
pub fn main(init: std.process.Init) !void {
    ...
    defer http.deinit();
    var provider = zigai.anthropic.Provider.init(key, http.transport());
    var client = zigai.anthropic.Client{ .model_name = "claude-sonnet-4-5", .provider = provider.provider() };
